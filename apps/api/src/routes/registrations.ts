...
router.post('/', zValidator('json', createRegistrationSchema), async (c) => {
  ...
  if (existingRegistration) {
    return c.json({ error: 'Participant already registered for this item' }, 400);
  }
...
router.patch('/:id/status', zValidator('json', updateRegistrationStatusSchema), async (c) => {
