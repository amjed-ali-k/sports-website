...
var _getQueryParam = /* @__PURE__ */ __name((url, key, multiple) => {
  ...
  while (keyIndex !== -1) {
    ...
    if (multiple) {
      ...
      results2[name] ?? (results2[name] = value);
    }
  }
  ...
  add(method, path, handler) {
    ...
    if (/\*$/.test(path)) {
      ...
      if (method === METHOD_NAME_ALL) {
        ...
        (_a107 = middleware[method])[path] || (_a107[path] = findMiddleware(middleware[method], path) || findMiddleware(middleware[METHOD_NAME_ALL], path) || []);
      }
      Object.keys(middleware).forEach((m) => {
...
var Node2 = /* @__PURE__ */ __name(class {
  ...
  search(method, path) {
    ...
    const results2 = handlerSets.sort((a, b) => {
      return a.score - b.score;
    });
    return [results2.map(({ handler, params }) => [handler, params])];
  }
}, "Node");
...
var updateRegistrationStatusSchema = z.object({
...
var router3 = hono().post("/", zodValidator(createRegistrationSchema), async (c) => {
  ...
  const registration = await db.insert(registrations).values(data).returning().get();
  ...
  }).from(registrations).leftJoin(participants, eq(registrations.participantId, participants.id)).where(eq(registrations.itemId, itemId)).all();
  ...
  const participantRegistrations = await db.select({
    registration: registrations,
    item: items
  }).from(registrations).leftJoin(items, eq(registrations.itemId, items.id)).where(eq(registrations.participantId, participantId)).all();
...
var router4 = hono().post("/", zodValidator(createResultSchema), async (c) => {
  ...
  await db.update(registrations).set({ status: "participated" }).where(eq(registrations.id, data.registrationId)).run();
  ...
  const itemResults = await db.select({
    ...
    registration: registrations,
    participant: participants
  }).from(results).leftJoin(registrations, eq(results.registrationId, registrations.id)).leftJoin(participants, eq(registrations.participantId, participants.id)).where(eq(results.itemId, itemId)).all();
  ...
  const participantResults = await db.select({
    ...
    registration: registrations,
    item: items
  }).from(results).leftJoin(registrations, eq(results.registrationId, registrations.id)).leftJoin(items, eq(results.itemId, items.id)).where(eq(registrations.participantId, participantId)).all();
  ...
  }).from(results).leftJoin(registrations, eq(results.registrationId, registrations.id)).leftJoin(participants, eq(registrations.participantId, participants.id)).groupBy(participants.id).orderBy(sql`total_points DESC`).all();
...
// src/index.ts
var api = hono().use("*", authMiddleware).route("/participants", participants_default).route("/items", items_default).route("/registrations", registrations_default).route("/results", results_default);
var app = hono().use("*", cors()).use(logger()).use("/api/*", async (c, next) => {
  c.set("db", createDb(c.env.DB));
  await next();
}).get("/", (c) => c.text("Sports Management API")).route("/api", api);
var src_default = app;
...
// .wrangler/tmp/bundle-wugn4k/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-wugn4k/middleware-loader.entry.ts
