...
var router = hono().post("/", zodValidator(createParticipantSchema), async (c) => {
  ...
  const latestParticipant = await db.select().from(participants).orderBy(participants.chestNo).get();
  const chestNo = latestParticipant ? latestParticipant.chestNo + 1 : 1e3;
...
var router3 = hono().post("/", zodValidator(createRegistrationSchema), async (c) => {
  ...
  }).from(registrations).innerJoin(participants, eq(registrations.participantId, participants.id)).innerJoin(items, eq(registrations.itemId, items.id)).all();
...
var router4 = hono().post("/", zodValidator(createResultSchema), async (c) => {
  ...
  const leaderboard = await db.select({
    ...
    totalPoints: sql`COALESCE(SUM(${results.points}), 0)`.as("total_points"),
    firstCount: sql`COALESCE(SUM(CASE WHEN ${results.position} = 'first' THEN 1 ELSE 0 END), 0)`.as("first_count"),
    secondCount: sql`COALESCE(SUM(CASE WHEN ${results.position} = 'second' THEN 1 ELSE 0 END), 0)`.as("second_count"),
    thirdCount: sql`COALESCE(SUM(CASE WHEN ${results.position} = 'third' THEN 1 ELSE 0 END), 0)`.as("third_count")
...
// .wrangler/tmp/bundle-a28KWo/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-a28KWo/middleware-loader.entry.ts
